  ...
  async validate(accessToken: string, refreshToken: string, profile: any, done: VerifyCallback) {
    try {
      ...
      profile = await firstValueFrom(
        this._httpService.get(url, options).pipe(map((response: AxiosResponse<any, any>) => response.data)),
      );

      /** Extract the userPrincipalName from the profile and create an emails array. */
      const emails = [{ value: profile.userPrincipalName, verified: !!profile.userPrincipalName }];

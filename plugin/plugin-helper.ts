...
export function getPluginExtensions(plugins: Array<Type<any> | DynamicModule>) {
  ...
  return plugins.map((plugin) => reflectMetadata(plugin, PLUGIN_METADATA.EXTENSIONS)).filter(isNotEmpty);
}
...
export function getDynamicPluginsModules(): DynamicModule[] {
  return getConfig()
    .plugins.map((plugin) => {
      ...
      const { imports, providers } = reflectDynamicModuleMetadata(pluginModule);
      return {
        module: pluginModule,
        imports,
        providers: [...providers],

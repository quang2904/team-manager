...
import { IChangelog, IChangelogCreateInput, IChangelogUpdateInput, IPagination } from 'contracts';
import { AuthGuard, CrudController, UUIDValidationPipe } from 'src/core';
...
export class ChangelogController extends CrudController<Changelog> {
  ...
  async update(
    ...
    return await this.commandBus.execute(new ChangelogUpdateCommand({ id, ...entity }));
  }
}

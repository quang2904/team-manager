...
export class GauzyAIService {
  ...
  public async syncGauzyEmployeeJobSearchCriteria(
    ...
    console.log(
      `syncGauzyEmployeeJobSearchCriteria called. Criteria: ${JSON.stringify(criteria)}. Employee: ${JSON.stringify(
        employee,
      )}`,
    );
  ...
  private async syncUser(user: User) {
    ...
    let userFilterByExternalFieldsQuery: DocumentNode<UserConnection> = gql`
      query userFilterByExternalFieldsQuery($externalUserIdFilter: String!, $externalTenantIdFilter: String!) {
        users(
          filter: { externalUserId: { eq: $externalUserIdFilter }, externalTenantId: { eq: $externalTenantIdFilter } }
        ) {
          edges {
            node {
              id
   ...
   * @returns
   */
  private async getTenantGauzyAIId(externalTenantId: string): Promise<string | null> {
    /** */
    let tenantByExternalTenantIdQuery: DocumentNode<TenantConnection> = gql`
      query tenantByExternalEmployeeId($externalTenantIdFilter: String!) {

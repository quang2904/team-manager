...
import { IEmployee, IOrganization, ITenant } from 'contracts';
import { getDefaultOrganizations, getDefaultEmployees, SeedDataService, Tenant, Employee } from 'src/core';
...
export class HelpCenterSeederService {
  ...
  async createDefault() {
    ...
    await this.seeder.tryExecute(
      ...
      createHelpCenterArticle(this.seeder.dataSource, [this.tenant], tenantOrganizationsMap, noOfHelpCenterArticle),
    );
  ...
  async createRandom() {
    ...
    await this.seeder.tryExecute(
      'Random Help Centers',
      createHelpCenter(this.seeder.dataSource, rendomTenants, tenantOrganizationsMap),
    );

    const noOfHelpCenterArticle = 40;
    await this.seeder.tryExecute(
      'Random Help Center Articles',
      createHelpCenterArticle(this.seeder.dataSource, rendomTenants, tenantOrganizationsMap, noOfHelpCenterArticle),
    );

    await this.seeder.tryExecute(
      'Random Help Center Authors',
      createRandomHelpCenterAuthor(this.seeder.dataSource, rendomTenants, employeeMap),
    );
  }
}

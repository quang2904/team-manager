  ...
  mapByEmployee(activities: IActivity[]) {
    const byEmployee: any = this.groupByEmployee(activities)
      .map((byEmployeeActivity: IActivity[]) => {
        ...
        const employee = byEmployeeActivity.length > 0 && byEmployeeActivity[0] ? byEmployeeActivity[0].employee : null;
        return {
          employee,
          dates: dailyLogs,
        };
  ...
  mapByProject(activities: IActivity[]) {
    const byEmployee: any = this.groupByProject(activities)
      .map((byProjectActivity: IActivity[]) => {
        ...
        const project = byProjectActivity.length > 0 && byProjectActivity[0] ? byProjectActivity[0].project : null;
        return {
          project,
          dates: dailyLogs,
        };

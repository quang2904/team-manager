...
import { IActivity, IScreenshot, ITimeLog } from 'contracts';
    ...
    if (timerSlots.length > 0) {
      const savePromises = _.chain(timerSlots)
        ...
        .mapObject(async (timeSlots, slotStart) => {
           ...
           */
          screenshots = screenshots.map((item) => new Screenshot(_.omit(item, ['timeSlotId'])));

          /*
           * Map old activities newly created TimeSlot
           */
          activities = activities.map((item) => new Activity(_.omit(item, ['timeSlotId'])));

...
import { ITimeLog, ITimesheet, TimeLogSourceEnum } from 'contracts';
  ...
  public async execute(command: TimeLogUpdateCommand): Promise<TimeLog> {
    ...
    if (needToUpdateTimeSlots) {
      const startTimes = timeSlots
        .filter((timeSlot) => {
          return updateTimeSlots.filter((newSlot) => moment(newSlot.startedAt).isSame(timeSlot.startedAt)).length === 0;
        })

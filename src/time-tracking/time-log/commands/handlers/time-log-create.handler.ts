...
import { TimeLogType, TimeLogSourceEnum, ITimeSlot } from 'contracts';
...
export class TimeLogCreateHandler implements ICommandHandler<TimeLogCreateCommand> {
  ...
  public async execute(command: TimeLogCreateCommand): Promise<TimeLog> {
    ...
    if (input.stoppedAt) {
      timeSlots = this.timeSlotService.generateTimeSlots(input.startedAt, input.stoppedAt).map((slot: ITimeSlot) => ({
        ...slot,
        employeeId,
        organizationId,
        tenantId,

...
import { IOrganizationContact, IReportDayGroupByClient, ITimeLog, ITimeSlot } from 'contracts';
import { GetTimeLogGroupByClientCommand } from '../get-time-log-group-by-client.command';
import { ArraySum } from 'common';
...
export class GetTimeLogGroupByClientHandler implements ICommandHandler<GetTimeLogGroupByClientCommand> {
  ...
  public async execute(command: GetTimeLogGroupByClientCommand): Promise<IReportDayGroupByClient> {
    ...
    const dailyLogs: any = chain(timeLogs)
      .groupBy((log: ITimeLog) => (log.organizationContact ? log.organizationContactId : null))
      .map((byClientLogs: ITimeLog[]) => {
        ...
        const avgActivity =
          (reduce(pluck(slots, 'overall'), ArraySum, 0) * 100) / reduce(pluck(slots, 'duration'), ArraySum, 0) || 0;

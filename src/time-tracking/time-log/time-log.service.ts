...
import { ArraySum, isEmpty, isNotEmpty } from 'common';
...
export class TimeLogService extends TenantAwareCrudService<TimeLog> {
    ...
    const weeklyLogs = chain(logs)
      ...
      .map((logs: ITimeLog[]) => {
        ...
        const weeklyActivity =
          (reduce(pluck(slots, 'overall'), ArraySum, 0) * 100) / reduce(pluck(slots, 'duration'), ArraySum, 0);
  ...
  async getDailyReport(request: IGetTimeLogReportInput) {
    ...
    switch (request.groupBy) {
      case ReportGroupFilterEnum.employee:
        dailyLogs = await this.commandBus.execute(new GetTimeLogGroupByEmployeeCommand(logs));
        break;
      case ReportGroupFilterEnum.project:
        dailyLogs = await this.commandBus.execute(new GetTimeLogGroupByProjectCommand(logs));
        break;
      case ReportGroupFilterEnum.client:
        dailyLogs = await this.commandBus.execute(new GetTimeLogGroupByClientCommand(logs));
        break;
      default:
        dailyLogs = await this.commandBus.execute(new GetTimeLogGroupByDateCommand(logs));
        break;
    }
      ...
      .map((byDateLogs: ITimeLog[], date: string) => {
        const byEmployee = chain(byDateLogs)
          ...
          .map((byEmployeeLogs: ITimeLog[]) => {
            const durationSum = byEmployeeLogs.reduce((iteratee: any, log: any) => {
              return iteratee + log.duration;
            }, 0);
  ...
  async getTimeLimit(request: IGetTimeLimitReportInput) {
    ...
    const byDate: any = chain(timeLogs)
      .groupBy((log) => {
        return moment(log.startedAt).startOf(request.duration).format('YYYY-MM-DD');
      })
      .mapObject((byDateLogs: ITimeLog[], date) => {
        const byEmployee = chain(byDateLogs)
          .groupBy('employeeId')
  ...
  async getClientBudgetLimit(request: IGetTimeLogReportInput) {
    ...
    return organizationContacts.map((organizationContact: IOrganizationContact): IClientBudgetLimitReport => {

...
import { IPluginConfig, isEmpty } from 'common';
...
export const createRandomTimeLogs = async (
  ...
  for (let timeSheetChunkIndex = 0; timeSheetChunkIndex < timeSheetChunk.length; timeSheetChunkIndex++) {
    ...
    for await (const timeSlot of trackedTimeSlots) {
      ...
      const randomScreenshots = await createRandomScreenshot(config, tenantId, organizationId, startedAt, stoppedAt);
      const screenshots = randomScreenshots.map((item) => new Screenshot(_.omit(item, ['timeSlotId'])));
...
export const recalculateTimesheetActivity = async (dataSource: DataSource, timesheets: ITimesheet[]) => {

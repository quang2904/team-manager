...
import { ArraySum, isNotEmpty } from 'common';
  ...
  async getMembers(request: IGetMembersStatistics): Promise<IMembersStatistics[]> {
    ...
    if (employees.length > 0) {
      ...
      weekTimeSlots = mapObject(groupBy(weekTimeSlots, 'employeeId'), function (values, employeeId) {
        ...
        const weekPercentage =
          (reduce(pluck(values, 'overall'), ArraySum, 0) * 100) / reduce(pluck(values, 'duration'), ArraySum, 0);
        return {
          employeeId,
  ...
  async getTasks(request: IGetTasksStatistics) {
    ...
    todayQuery
      ...
      .andWhere(
        new Brackets((qb: WhereExpressionBuilder) => {
          if (todayStart && todayEnd) {
            qb.andWhere(`"${todayQuery.alias}"."startedAt" BETWEEN :todayStart AND :todayEnd`, {
              todayStart,
              todayEnd,
            });
            qb.andWhere(`"time_slot"."startedAt" BETWEEN :todayStart AND :todayEnd`, {
              todayStart,
              todayEnd,
            });
          }
      ...
      .andWhere(
        new Brackets((qb: WhereExpressionBuilder) => {
          ...
          if (isNotEmpty(projectIds)) {
            qb.andWhere(`"${todayQuery.alias}"."projectId" IN (:...projectIds)`, {
              projectIds,
            });
          }
          if (isNotEmpty(taskIds)) {
            qb.andWhere(`"${todayQuery.alias}"."taskId" IN (:...taskIds)`, {
              taskIds,
            });
          }
          if (isNotEmpty(organizationTeamId)) {
            qb.andWhere(`"${todayQuery.alias}"."organizationTeamId" = :organizationTeamId`, {
              organizationTeamId,
            });
          }
        }),
      ...
      .addGroupBy(`"task"."id"`)
      .orderBy(`"${todayQuery.alias}"."updatedAt"`, 'DESC');
    ...
    tasks = tasks.map((task: any) => {
      task.durationPercentage = parseFloat(parseFloat((task.duration * 100) / totalDuration.duration + '').toFixed(2));
      return task;
    });
    return tasks || [];
  }
      ...
      .andWhere(
        new Brackets((qb) => {
          if (this.configService.dbConnectionOptions.type === 'sqlite') {
            qb.andWhere(`datetime("${query.alias}"."date" || ' ' || "${query.alias}"."time") Between :start AND :end`, {
              start,
              end,
            });
          } else {

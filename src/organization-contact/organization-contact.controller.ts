...
import { IEditEntityByMemberInput, IEmployee, IOrganizationContact, IPagination, PermissionsEnum } from 'contracts';
...
export class OrganizationContactController extends CrudController<OrganizationContact> {
  ...
  async pagination(@Query() filter: PaginationParams<OrganizationContact>): Promise<IPagination<IOrganizationContact>> {

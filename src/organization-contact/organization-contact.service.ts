...
import { IEmployee, IOrganizationContact, IOrganizationContactFindInput, IPagination } from 'contracts';
  ...
  async getOrganizationContactByEmployee(data: any) {
    ...
    query.where(
      new Brackets((subQuery) => {
        subQuery.where('members.id =:employeeId', { employeeId }).orWhere(`${query.alias}.createdBy = :createdBy`, {
          createdBy,
        });
      }),
    );

...
import { isEmpty, isNotEmpty, isObject } from 'common';
import {
  IIntegrationTenant,
  IntegrationEnum,
  IntegrationEntity,
...
} from 'plugins/integration-hubstaff';
...
export class HubstaffService {
   ...
   */
  async fetchOrganizationProjects({ organizationId, token }): Promise<IHubstaffProject[]> {
   ...
   * @returns
   */
  async syncProjects({ integrationId, organizationId, projects, token }): Promise<IIntegrationMap[]> {
    try {
      const tenantId = RequestContext.currentTenantId();
      return await Promise.all(
        await projects.map(async ({ sourceId }) => {
   ...
   * @returns
   */
  async syncOrganizations({ integrationId, organizationId, organizations, token }): Promise<IIntegrationMap[]> {
    try {
      const tenantId = RequestContext.currentTenantId();
      return await Promise.all(
        await organizations.map(async ({ sourceId }) => {
  ...
  async syncClients({ integrationId, organizationId, clients }): Promise<IIntegrationMap[]> {
    try {
      return await Promise.all(
        await clients.map(async ({ id, name, emails, phone, budget = {} as any }) => {
   ...
   */
  async syncScreenshots({ integrationId, screenshots, token, organizationId }): Promise<IIntegrationMap[]> {
    try {
  ...
  async syncTasks({ integrationId, projectId, tasks, organizationId }): Promise<IIntegrationMap[]> {
    try {
      const tenantId = RequestContext.currentTenantId();
      const creatorId = RequestContext.currentUserId();

      return await Promise.all(
        await tasks.map(async ({ summary: title, details = null, id, status, due_at }) => {
          if (!due_at) {
  ...
  private async _handleEmployee({ user_id, integrationId, token, organizationId }) {
    try {
  ...
  private async _handleProjects(sourceId: string, integrationId: string, gauzyId: string, token: string) {
    try {
  ...
  private async _handleClients(sourceId: string, integrationId: string, gauzyId: string, token: string) {
    try {
   ...
   */
  async syncUrlActivities({ integrationId, projectId, activities, token, organizationId }): Promise<IIntegrationMap[]> {
    try {
      const tenantId = RequestContext.currentTenantId();
      return await Promise.all(
   ...
   */
  async syncAppActivities({ integrationId, projectId, activities, token, organizationId }): Promise<IIntegrationMap[]> {
    try {
      const tenantId = RequestContext.currentTenantId();
      return await Promise.all(
      ...
      entitySettings.map(async (setting) => {
        switch (setting.entity) {
          case IntegrationEntity.PROJECT:
            ...
            const projectsMap: IIntegrationMap[] = await this._handleProjects(sourceId, integrationId, gauzyId, token);

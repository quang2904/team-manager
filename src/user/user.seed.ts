...
export const createDefaultAdminUsers = async (
  ...
  const [defaultSuperAdminUsers, defaultAdminUsers] = await Promise.all([_defaultSuperAdminUsers, _defaultAdminUsers]);
...
export const createRandomUsers = async (
  ...
  for (const tenant of tenants) {
    ...
    const [
      ...
      promiseViewerUsers,
    ] = await Promise.all([_adminUsers, _employeeUsers, _candidateUsers, _managerUsers, _dataEntryUsers, _viewerUsers]);
...
const generateRandomUser = async (role: IRole, tenant: ITenant): Promise<IUser> => {
  ...
  user.preferredLanguage = languages[Math.floor(Math.random() * languages.length)];

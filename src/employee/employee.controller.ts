import { PermissionsEnum, LanguagesEnum, IPagination, IEmployee } from 'contracts';
import {
  BadRequestException,
  Body,
  Controller,
...
export class EmployeeController extends CrudController<Employee> {
  ...
  async findAllWorkingEmployeesCount(@Query('data', ParseJsonPipe) data: any): Promise<{ total: number }> {
    ...
    return await this.employeeService.findWorkingEmployeesCount(organizationId, forRange, withUser);
  }
  ...
  @Get('pagination')
  @UsePipes(new ValidationPipe({ transform: true }))
  async pagination(@Query() options: PaginationParams<Employee>): Promise<IPagination<IEmployee>> {

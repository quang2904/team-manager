import { ComponentLayoutStyleEnum, IEmployee, LanguagesEnum, RolesEnum } from 'contracts';
  ...
  public async execute(command: EmployeeCreateCommand): Promise<IEmployee> {
    ...
    if (isEmpty(input.userId)) {
      ...
      if (employee.organizationId) {
        await this._userOrganizationService.addUserToOrganization(employee.user, employee.organizationId);
      }

      // 4. Send welcome email to user register employee
      this._emailService.welcomeUser(employee.user, languageCode, employee.organizationId, originUrl);
      return employee;
    } else {
      try {
        const { userId } = input;

...
import { IDeal, IPagination, IPipeline, PermissionsEnum } from 'contracts';
...
export class PipelineController extends CrudController<Pipeline> {
  ...
  @UseGuards(PermissionGuard)
  @Get()
  public async findAll(@Query('data', ParseJsonPipe) data: any): Promise<IPagination<IPipeline>> {
  ...
  @UseGuards(PermissionGuard)
  @Post()
  async create(@Body() entity: DeepPartial<Pipeline>, ...options: any[]): Promise<IPipeline> {

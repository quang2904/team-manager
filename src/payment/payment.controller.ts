...
export class PaymentController extends CrudController<Payment> {
  constructor(private readonly paymentService: PaymentService, private readonly paymentMapService: PaymentMapService) {
  ...
  @UsePipes(new ValidationPipe({ transform: true, whitelist: true }))
  async getDailyReportChartData(@Query() options: PaymentReportQueryDTO) {

      ...
      .map((byDatePayment: IPayment[], date) => {
        ...
        const byClient = this.groupByClient(byDatePayment)
          .map((byClientPayment: IPayment[]) => {
            ...
            const employee = byClientPayment.length > 0 && byClientPayment[0] ? byClientPayment[0].employee : null;
            return {
              employee,
              projects: byProject,
            };
  ...
  mapByClient(payments: IPayment[]): IPaymentReportGroupByClient[] {
    const byClient: any = this.groupByClient(payments)
      .map((byClientPayment: IPayment[]) => {
        ...
        const client = byClientPayment.length > 0 && byClientPayment[0] ? byClientPayment[0].organizationContact : null;
        return {
          client,
          dates: dailyLogs,
        };

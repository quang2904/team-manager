...
export const createDefaultOrganizations = async (
  ...
  organizations.forEach((organization: IOrganizationCreateInput) => {
    ...
    defaultOrganization.timeZone = faker.helpers.arrayElement(timezone.tz.names().filter((zone) => zone.includes('/')));
  ...
  for await (const tenant of tenants) {
    ...
    if (tenant.name === 'Ever') {
      ...
      for (let index = 0; index < organizationsPerTenant; index++) {
        ...
        organization.brandColor = faker.helpers.arrayElement(['#FF0000', '#008000', '#0000FF', '#FFA500', '#FFFF00']);
        organization.contact = faker.helpers.arrayElement(contacts);
        organization.timeZone = faker.helpers.arrayElement(timezone.tz.names().filter((zone) => zone.includes('/')));

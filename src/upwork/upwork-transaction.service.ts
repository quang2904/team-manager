...
import { OrganizationVendorEnum, ExpenseCategoriesEnum, IncomeTypeEnum } from 'contracts';
    ...
    return new Promise((resolve, reject) => {
      csvReader.on('end', async () => {
        ...
        const transactions = results
          .filter(
            (result) => result.Type === IncomeTypeEnum.HOURLY || result.Type === ExpenseCategoriesEnum.SERVICE_FEE,
          )
          .map(async (result) => {
      ...
      (prev, current) => {
        return {
          rejectedTransactions:
            current.status === 'rejected' ? prev.rejectedTransactions.concat(current) : prev.rejectedTransactions,
          totalExpenses:
            current.item instanceof Expense ? (prev.totalExpenses++, prev.totalExpenses) : prev.totalExpenses,
          totalIncomes: current.item instanceof Income ? (prev.totalIncomes++, prev.totalIncomes) : prev.totalIncomes,
        };
      },
      {
        rejectedTransactions: [],

...
import { RolesEnum, LanguagesEnum, IUser } from 'contracts';
...
export class UserOrganizationDeleteHandler implements ICommandHandler<UserOrganizationDeleteCommand> {
  ...
  public async execute(command: UserOrganizationDeleteCommand): Promise<UserOrganization | DeleteResult> {
    ...
    const {
      ...
      userId,
    } = await this.userOrganizationService.findOneByIdString(input.userOrganizationId, {
      relations: ['user', 'user.role'],
    });

    // 2. Handle Super Admin Deletion if applicable
    if (roleName === RolesEnum.SUPER_ADMIN) return this._removeSuperAdmin(input.requestingUser, userId, input.language);
  ...
  private async _removeUserFromOrganization(
    ...
    return total === 1 ? this.userService.delete(userId) : this.userOrganizationService.delete(userOrganizationId);
  }
  ...
  private async _removeSuperAdmin(
    ...
    if (requestingUserRoleName !== RolesEnum.SUPER_ADMIN)
      throw new UnauthorizedException('Only Super Admin user can delete Super Admin users');
    ...
    if (total === 1)
      throw new BadRequestException(
        await this.i18n.translate('USER_ORGANIZATION.CANNOT_DELETE_ALL_SUPER_ADMINS', {
          lang: language,
          args: { count: 1 },
        }),
      );

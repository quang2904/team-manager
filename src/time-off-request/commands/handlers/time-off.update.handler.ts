...
import { StatusTypesMapRequestApprovalEnum, RequestApprovalStatusTypesEnum } from 'contracts';
...
export class TimeOffUpdateHandler implements ICommandHandler<TimeOffUpdateCommand> {
  ...
  public async execute(command?: TimeOffUpdateCommand): Promise<TimeOffRequest> {
    try {
      ...
      const timeOffRequestSaved = await this.timeOffRequestRepository.save(timeOff);

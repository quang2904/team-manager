...
import { IOrganization, IEmployee, ITenant, ContactType, OrganizationContactBudgetTypeEnum, IIncome } from 'contracts';
...
export const createDefaultIncomes = async (
  ...
  try {
    filePath = fs.existsSync(filePath) ? filePath : `./income-seed-data/income-data.csv`;
  } catch (error) {
...
const insertIncome = async (dataSource: DataSource, incomes: Income[]): Promise<void> => {

...
import { IAppIntegrationConfig, isNotEmpty } from 'common';
...
export class InviteService extends TenantAwareCrudService<Invite> {
  ...
  async createOrganizationContactInvite(inviteInput: ICreateOrganizationContactInviteInput): Promise<Invite> {
    ...
    const inviteExpiryPeriod =
      organization && organization.inviteExpiryPeriod ? organization.inviteExpiryPeriod : DEFAULT_INVITE_EXPIRY_PERIOD;
  ...
  async createUser(
    ...
    const { appName, appLogo, appSignature, appLink, companyLink, companyName } = input;

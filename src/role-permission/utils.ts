      ...
      for await (const role of roles) {
        ...
        for await (const permission of permissions) {
          ...
          if (isEmpty(existed) && role) {
            console.log(
              chalk.magenta(
                `${moment().format('DD.MM.YYYY HH:mm:ss')} unlocked missing permission for the tenant: ${tenant.name}`,
                roleEnum,
                permission,
              ),
            );
            ...
            console.log(chalk.magenta([tenantId, roleId, permission, defaultEnabledPermissions.includes(permission)]));
            /**

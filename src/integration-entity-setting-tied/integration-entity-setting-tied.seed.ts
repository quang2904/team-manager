...
import { IIntegrationEntitySettingTied, IntegrationEntity, ITenant } from 'contracts';
import { PROJECT_TIED_ENTITIES } from 'plugins/integration-hubstaff';
    ...
    for (const integrationTenant of integrationTenants) {
      ...
      for (const integrationEntitySetting of integrationEntitySettings) {
        ...
        if (integrationEntitySetting['entity'] === IntegrationEntity.PROJECT) {
          integrationEntitySettingTiedEntity.entity = faker.helpers.arrayElement(PROJECT_TIED_ENTITIES)['entity'];
        } else {
          integrationEntitySettingTiedEntity.entity = faker.helpers.arrayElement(Object.values(IntegrationEntity));
        }

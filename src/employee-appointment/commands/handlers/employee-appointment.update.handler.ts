...
export class EmployeeAppointmentUpdateHandler implements ICommandHandler<EmployeeAppointmentUpdateCommand> {
  ...
  public async execute(command?: EmployeeAppointmentUpdateCommand): Promise<UpdateResult | EmployeeAppointment> {
    ...
    const employee = employeeAppointmentUpdateRequest.employeeId
      ? await this.employeeService.findOneByIdString(employeeAppointmentUpdateRequest.employeeId)
      : null;
    const organization = employeeAppointmentUpdateRequest.organizationId
      ? await this.organizationService.findOneByIdString(employeeAppointmentUpdateRequest.organizationId)
      : null;

    employee && (appointment.employee = employee);
    organization && (appointment.organization = organization);
    employeeAppointmentUpdateRequest.agenda && (appointment.agenda = employeeAppointmentUpdateRequest.agenda);
    ...
    employeeAppointmentUpdateRequest.bufferTimeEnd &&
      (appointment.bufferTimeEnd = employeeAppointmentUpdateRequest.bufferTimeEnd);
    employeeAppointmentUpdateRequest.bufferTimeInMins &&
      (appointment.bufferTimeInMins = employeeAppointmentUpdateRequest.bufferTimeInMins);
    employeeAppointmentUpdateRequest.breakStartTime &&
      (appointment.breakStartTime = employeeAppointmentUpdateRequest.breakStartTime);
    employeeAppointmentUpdateRequest.breakTimeInMins &&
      (appointment.breakTimeInMins = employeeAppointmentUpdateRequest.breakTimeInMins);
    employeeAppointmentUpdateRequest.bufferTimeStart &&
      (appointment.bufferTimeStart = employeeAppointmentUpdateRequest.bufferTimeStart);
    employeeAppointmentUpdateRequest.startDateTime &&
      (appointment.startDateTime = employeeAppointmentUpdateRequest.startDateTime);
    employeeAppointmentUpdateRequest.endDateTime &&
      (appointment.endDateTime = employeeAppointmentUpdateRequest.endDateTime);
    employeeAppointmentUpdateRequest.location && (appointment.location = employeeAppointmentUpdateRequest.location);
    employeeAppointmentUpdateRequest.status && (appointment.status = employeeAppointmentUpdateRequest.status);

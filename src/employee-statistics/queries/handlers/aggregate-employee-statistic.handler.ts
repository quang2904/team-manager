...
export class AggregateOrganizationQueryHandler implements IQueryHandler<AggregatedEmployeeStatisticQuery> {
  constructor(private employeeService: EmployeeService, private employeeStatisticsService: EmployeeStatisticsService) {}
  ...
  private async _loadOrganizationRecurringSplitExpenses(
    ...
    //Since we are only calculating for one month, we only expect one value here.
    const monthSplitExpense: IMonthAggregatedSplitExpense = organizationRecurringSplitExpenses.values().next().value;

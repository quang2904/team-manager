...
import { sluggable } from 'common';
import { FileStorageProviderEnum } from 'contracts';
...
export class TaskPrioritySubscriber implements EntitySubscriberInterface<TaskPriority> {
   ...
   * @param event
   */
  afterLoad(entity: TaskPriority | Partial<TaskPriority>, event?: LoadEvent<TaskPriority>): void | Promise<any> {
    try {
      if (entity.icon) {

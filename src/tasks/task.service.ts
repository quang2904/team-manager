...
import { IsNull, Repository, SelectQueryBuilder, Brackets, WhereExpressionBuilder, Raw, FindOneOptions } from 'typeorm';
import { IEmployee, IGetTaskOptions, IPagination, ITask, PermissionsEnum } from 'contracts';
...
export class TaskService extends TenantAwareCrudService<Task> {
  ...
  async findById(id: ITask['id'], params: GetTaskByIdDTO): Promise<ITask> {
    const task = await this.findOneByIdString(id, params as FindOneOptions);
        ...
        query.andWhere(
          new Brackets((web: WhereExpressionBuilder) => {
            web.andWhere((qb: SelectQueryBuilder<Task>) => {
              const subQuery = qb.subQuery();
              subQuery.select('"task_team"."taskId"').from('task_team', 'task_team');
              subQuery.andWhere('"task_teams"."organizationTeamId" = :organizationTeamId', { employeeId });
              return '"task_teams"."taskId" IN ' + subQuery.distinct(true).getQuery();
            });
          }),
        );

...
import { PermissionsEnum, IInvoice, LanguagesEnum, IPagination } from 'contracts';
...
export class InvoiceController extends CrudController<Invoice> {
  ...
  async updateEstimate(@Param('id', UUIDValidationPipe) id: IInvoice['id'], @Body() entity: UpdateEstimateInvoiceDTO) {
    return await this.commandBus.execute(new InvoiceUpdateCommand({ id, ...entity }));
  }

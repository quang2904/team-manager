...
import { environment as env, ConfigService } from 'config';
import { IEmployee, IOrganization, IOrganizationProject, IRole, ITenant, IUser } from 'contracts';
import { getPluginModules, hasLifecycleMethod, PluginLifecycleMethods } from 'plugin';
...
export class SeedDataService {
  ...
  private async seedBasicDefaultData() {
    ...
    const { defaultSuperAdminUsers, defaultAdminUsers } = await createDefaultAdminUsers(this.dataSource, this.tenant);
    ...
    await this.tryExecute('Default Key Result Template', createDefaultKeyResultTemplates(this.dataSource, this.tenant));
  ...
  private async seedDefaultData() {
    ...
    await this.tryExecute(
      'Default Contacts',
      createRandomContacts(this.dataSource, this.tenant, this.organizations, randomSeedConfig.noOfRandomContacts || 5),
    );
    ...
    await this.tryExecute(
      ...
      createDefaultEmployeeAppointment(this.dataSource, this.tenant, this.defaultEmployees, this.defaultOrganization),
    );
    ...
    await this.tryExecute(
      ...
      createDefaultCandidateTechnologies(this.dataSource, this.tenant, this.defaultOrganization, defaultCandidates),
    );

    await this.tryExecute(
      'Default Candidate Criterion Rating',
      createDefaultCandidateCriterionRating(this.dataSource, this.tenant, this.defaultOrganization, defaultCandidates),
    );
  ...
  private async seedRandomData() {
    ...
    await this.tryExecute('Random Feature Reports', createRandomTenantOrganizationsReport(this.dataSource, tenants));
    ...
    await this.tryExecute('Random Merchants', createRandomMerchants(this.dataSource, tenants, tenantOrganizationsMap));
    ...
    await this.tryExecute(
      'Random Key Results',
      createRandomKeyResult(this.dataSource, tenants, randomGoals, tenantOrganizationsMap, organizationEmployeesMap),
    );
    ...
    await this.tryExecute(
      ...
      createRandomEmployeeRecurringExpense(this.dataSource, tenants, tenantOrganizationsMap, organizationEmployeesMap),
    );
    ...
    await this.tryExecute(
      ...
      createRandomOrganizationTeamEmployee(this.dataSource, tenants, tenantOrganizationsMap, organizationEmployeesMap),
    );
    ...
    await this.tryExecute('Random Pipelines', createRandomPipeline(this.dataSource, tenants, tenantOrganizationsMap));
    ...
    await this.tryExecute(
      ...
      createRandomRequestApprovalEmployee(this.dataSource, tenants, tenantOrganizationsMap, organizationEmployeesMap),
    );

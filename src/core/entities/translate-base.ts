import { ITranslation, ITranslatable, TranslatePropertyInput, TranslateInput } from 'contracts';
...
export abstract class TranslatableBase extends TenantOrganizationBaseEntity implements ITranslatable<TranslationBase> {
  ...
  translateNested(languageCode: string, translatePropsInput: Array<TranslatePropertyInput>): any {
    ...
    let inputProps: Array<TranslatePropertyInput> = translatePropsInput.map((translateObj: TranslatePropertyInput) => {
      return {
        ...translateObj,
    ...
    Object.keys(this).forEach((prop: string) => {
      ...
      if (!currentInputProp) {
        ...
        if (prop == 'root') {
          ...
          elementPropTranslations = element[inputKeys[0]] ? element[inputKeys[0]][inputKeys[1]].translations : [];
        }

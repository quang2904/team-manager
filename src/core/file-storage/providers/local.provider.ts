...
import { FileStorageOption, FileStorageProviderEnum, UploadedFile } from 'contracts';
...
export class LocalProvider extends Provider<LocalProvider> {
  ...
  config = {
    rootPath:
      (environment.isElectron ? resolve(environment.gauzyUserPath, 'public') : config.assetOptions.assetPublicPath) ||

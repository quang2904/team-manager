...
import { IOrganization, IEmployee, IExpenseCategory, IOrganizationVendor, ITenant } from 'contracts';
...
export const createDefaultExpenses = async (
  ...
  try {
    filePath = fs.existsSync(filePath) ? filePath : `./expense-seed-data/expenses-data.csv`;
  } catch (error) {
...
export const createRandomExpenses = async (
  ...
  const notesArray = ['Windows 10', 'MultiSport Card', 'Angular Masterclass', 'Drive', 'Rent for September'];
    ...
    for (const organization of organizations) {
      ...
      for (const employee of employees) {
        ...
        for (let index = 0; index < 100; index++) {
          ...
          expense.vendor = organizationVendors[currentIndex % organizationVendors.length];
...
const insertExpense = async (dataSource: DataSource, expenses: Expense[]): Promise<void> => {

...
import { IDateRangePicker, IExpense, IGetExpenseInput, IPagination, PermissionsEnum } from 'contracts';
...
export class ExpenseService extends TenantAwareCrudService<Expense> {
  ...
  public async findAllExpenses(filter?: FindManyOptions<Expense>, filterDate?: string): Promise<IPagination<Expense>> {
    if (filterDate) {
